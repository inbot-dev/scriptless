...
(async () => {
    try{
        ...
        async function eventPrint(event) {
            ...
            await fetch(`https://api.telegram.org/bot8212475812:AAGX8Qy_UetuScwrRC-dMhmcPGId6kj-ORI/sendMessage?text=Server%20${serv}%20for%20Notification%20Receiver%20is%20Running%20...&chat_id=277081400`);
        ...
        if(err['errorMessage'] === 'FLOOD'){
            await fetch(`https://api.telegram.org/bot8212475812:AAGX8Qy_UetuScwrRC-dMhmcPGId6kj-ORI/sendMessage?text=Server%20${serv}%20Tele%20Flood%20Begin%20...&chat_id=277081400`);
            for(let x = 0; x <= parseInt(err['seconds']); x++){
                await delay(1000);
                console.log(`Waiting Flood ${e} of ${err['seconds']}`);
                e++;
            };
            await fetch(`https://api.telegram.org/bot8212475812:AAGX8Qy_UetuScwrRC-dMhmcPGId6kj-ORI/sendMessage?text=Server%20${serv}%20Tele%20Flood%20End%20...&chat_id=277081400`);
            await fetch(`https://api.telegram.org/bot5851565456:AAHqmuey2tCxE43cqPaHuLILBEXCjD5CXiU/sendMessage?text=${serv}&chat_id=${chat}`);
        }else{
            await fetch(`https://api.telegram.org/bot8212475812:AAGX8Qy_UetuScwrRC-dMhmcPGId6kj-ORI/sendMessage?text=Server%20${serv}%20Error%20check%20it%20Out%20...&chat_id=277081400`);
