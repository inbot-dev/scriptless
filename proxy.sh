#!/bin/bash
# proxy-init.sh — Ubuntu 24.04 bootstrap
...
log() { echo "[proxy-init] $*"; }
warn() { echo "[proxy-init] WARNING: $*" >&2; }
die() { echo "[proxy-init] ERROR: $*" >&2; exit 1; }
...
configure_squid() {
  ...
  cat > "${SQUID_CONF}" <<EOF
# Generated by proxy-init.sh — auth forward proxy (do not manage via Webmin Squid UI without re-running this)
