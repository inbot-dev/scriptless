...
if ! screen -list | grep -q "ws_work_wss-one"; then
    if ! screen -list | grep -q "ws_work_wss-one_de"; then
        ...
        screen -dmS ws_work_wss-one rclone sync --local-no-check-updated --drive-acknowledge-abuse --progress --checksum --metadata ws-onedrive_work:IE/S2 wss-onedrive:S2
    fi
fi
